...
def _write_match_html(jobs: list[dict], date_posted: str, skill_count: int) -> str:
    ...
    rows = ""
    for i, j in enumerate(jobs, 1):
        matched = html.escape(", ".join(j["matched_skills"][:10]) or "-")
        ...
        rows += (
            ...
            f'<td style="padding:8px;border-bottom:1px solid #eee;"><a href="{link}" '
            f'style="color:#0a66c2;text-decoration:none;">{html.escape(j["title"] or "")}</a></td>'
    ...
    doc = (
        ...
        f'<p style="color:#666;margin-top:0;">{len(jobs)} jobs (window: {html.escape(date_posted)}) '
        f'scored against your resume - {skill_count} skills detected. '
        f'Generated {now:%Y-%m-%d %H:%M}. Sorted best fit first.</p>'
        ...
        '<p style="color:#888;font-size:12px;margin-top:14px;">Weighted skill-coverage score with a '
        'denominator floor and a penalty per missing knockout skill. A heuristic first pass - it does '
        'not model seniority or non-skill credential gates. Generated by the job-search MCP.</p>'
        '</body></html>'
    )
...
def match_jobs(date_posted: str | None = None, write_log: bool = True,
    ...
    total = len(scored)
    scored, suppressed = _apply_only_new(scored, only_new)
    result = {
        "date_posted": dp,
    ...
    if write_log:
        result["logfile"] = _write_match_log(scored, dp, len(res_skills))
        result["html_report"] = _write_match_html(scored, dp, len(res_skills))
    return json.dumps(result, indent=2, default=str)
